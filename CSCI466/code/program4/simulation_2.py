...
if __name__ == '__main__':
    ...
    cost_D = {'H1': {0: 3}, 'H2': {1: 5}, 'RB': {2: 1}, 'RC': {3: 2}} # {neighbor: {interface: cost}}
    ...
    router_b = network.Router(name='RB',
                              cost_D = cost_D,
                              max_queue_size=router_queue_size)
    cost_D = {'RA': {0: 2}, 'RD': {1: 5}} #DL: Empty
    router_c = network.Router(name='RC',
                                cost_D = cost_D,
                                max_queue_size=router_queue_size)

    cost_D = {'RB': {0: 1}, 'RC': {1: 5}, 'H3': {2: 2}} # packet to host 2 through interface 1 for cost 3
    ...
    link_layer.add_link(link.Link(host_2, 0, router_a, 1))##FORWARD
    #DL: Tier 2
    ...
    link_layer.add_link(link.Link(router_b, 2, router_a, 1))
    link_layer.add_link(link.Link(router_a, 2, host_1, 0))
